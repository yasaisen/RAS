...
<html>
    ...
    <script type="text/javascript">
        ...
        {
        document.getElementById('attendance_record_name').value='';
        document.getElementById('attendance_record_start_date').value='';
        document.getElementById('attendance_record_end_date').value='';
        }
        function check_time()
        {
            var name = $("#attendance_record_name").val();
            var start = $("#attendance_record_start_date").val();
            var end = $("#attendance_record_end_date").val();
            if(name = "")
            {
                alert("請輸入查詢姓名");
            }
            if(start!="" & end!="")
            {
                ...
                if(dayCount>=0)
                {
                document.attendance_record_search.submit();
                }
                else
                {
                    alert("請選擇正確的時間範圍");
                ...
                <div class="title">
                    <div class="container">
                        <div class="row">
                            <div class="col">
                                <!-- 功能標題 -->
                                <b style="font-family:'Times New Roman','標楷體';">病人出席紀錄</b>
                            </div> 
                                        ...
                                        <script>
                                            ...
                                            function popup(e) 
                                            {
                                                if (confirm('確定要登出嗎') == true) {
                                                    ...
                                                    window.location.href="attendance_record.php" 
                                                }
                                            };
                ...
                <div>
                    <div class="container" >
                        <div class="row align-items-center">
                            <form action="./attendance_record_session.php" method="post" id="attendance_record_search" name="attendance_record_search">
                                <div class="col attendance_record_search_font" >
                                    查詢姓名：
                                    <!--查詢姓名輸入框-->
                                    <input type="text" id="attendance_record_name" name="attendance_record_name" maxlength="3" value="<?php echo $_SESSION['attendance_record_name'] ?>"
                                    ...
                                    <?php session_start(); ?>
                                        開始時間&nbsp;
                                    <!--datetimepicker-->
                                    <input type="date" id="attendance_record_start_date" name="attendance_record_start_date" value="<?php echo $_SESSION['attendance_record_start_date'] ?>"
                                        style="font-size:15px; font-family:'Times New Roman','標楷體'; padding: 0px;">
                                    &nbsp;&nbsp;
                                        結束時間&nbsp;
                                    <!--datetimepicker-->
                                    <input type="date" id="attendance_record_end_date" name="attendance_record_end_date" value="<?php echo $_SESSION['attendance_record_end_date'] ?>" 
                                        style="font-size:15px; font-family:'Times New Roman','標楷體'; padding: 0px;">
                                    &nbsp;&nbsp;&nbsp;&nbsp;
                                    <button class="attendance_record_page_btn" onclick="check_time(); check(attendance_record_name.value);">查詢</button>
                                </div>      
                            </form > 
                        </div>
                    </div>
                ...
                <div class="container" >
                    <div class="row align-items-center">
                        <div class="col" style="text-align:center; font-family:'Times New Roman','標楷體';">
                            <table id="attendance_record_table" class="display attendance_record_table">
                                <thead>
                                    <tr style="background-color: rgb(165,165,165); border-color:rgb(165,165,165); border-width: 1px;">
                                        <th class="text-center">日期(年月日)</th>
                                        <th>時段</th>
                                        <th>姓名</th>
                                        <th>出缺席</th>
                                    </tr>
                                ...
                                <tbody>
                                    ...
                                    <?php
                                        ...
                                        $name=$_SESSION['attendance_record_name'];
                                        $start_date=$_SESSION['attendance_record_start_date'];
                                        $end_date=$_SESSION['attendance_record_end_date'];
                                        if( $name == '' ) {
                                            $sql = "SELECT `reserve`.`date`, `pTime`.`pTimeStart`, `pTime`.`pTimeEnd`, `patient`.`patientName`, `reserve`.`state` 
                                                FROM 
                                                    confirm RIGHT OUTER JOIN reserve ON reserve.reserveId = confirm.reserveId 
                                                    JOIN pTime ON reserve.pTimeId = pTime.pTimeId 
                                                    JOIN patient ON reserve.patientId = patient.patientId 
                                                where (reserve.date between '$start_date' and '$end_date') AND(
                                                    reserve.state!='true' OR reserve.state!='false'
                                                    );";
                                            $sql = "
                                                select reserve.date, pTime.pTimeStart, pTime.pTimeEnd, patient.patientName, reserve.state 
                                                from 
                                                    confirm RIGHT OUTER JOIN reserve ON reserve.reserveId = confirm.reserveId 
                                                    JOIN pTime ON reserve.pTimeId = pTime.pTimeId 
                                                    JOIN patient ON reserve.patientId = patient.patientId 
                                            ...
                                            $sql = "
                                                ...
                                                and patient.patientName='$name' AND(
                                                    reserve.state!='true' OR reserve.state!='false'
                                                    );";
                                        }
                                        //echo $sql;
                                        ...
                                        $sendbackarray = array();
                                        while ($catch = $result->fetch_assoc()){
                                            $attendance_record_pTimeStart = explode(':', $catch['pTimeStart'])[0].':'.explode(':', $catch['pTimeStart'])[1];
                                            $attendance_record_pTimeEnd = explode(':', $catch['pTimeEnd'])[0].':'.explode(':', $catch['pTimeEnd'])[1];
                                            echo '<tr style="border-color:rgb(165,165,165); border-width: 1px;">';
                                    ?>
                                        <td><?php echo $catch['date']; ?></td>
                                        <td><?php echo $attendance_record_pTimeStart ?>~<?php echo $attendance_record_pTimeEnd ?></td>
                                        <td><?php echo $catch['patientName']; ?></td>
                                        <td><?php 
                                                if( $catch['confirmId'] == '' && ($catch['state'] == 'unlook' || $catch['state'] == 'passed')) { 
                                                    echo '<p style="color:red; margin: 0px;">未到</p>'; 
                                                }else { 
                                                    echo '出席'; 
                                                }
                                            ?>
                                    ...
                                    <div class="modal-content">
                                            ...
                                            <div class="container-fluid">
                                                <div class="row" style=" margin-top: 10px; margin-bottom: 10px;">
                                                    ...
                                                    <div class="col" style="justify-content:center; font-family:'Times New Roman','標楷體';">
                                                        <?php 
                                                            if($_SESSION['attendance_record_start_date'] == null){
                                                                echo '未指定開始日期';
                                                            }else{
                                                ...
                                                <div class="row" style=" margin-top: 10px; margin-bottom: 10px;">
                                                    ...
                                                    <div class="col" style="justify-content:center; font-family:'Times New Roman','標楷體';">
                                                        <?php 
                                                            if($_SESSION['attendance_record_end_date'] == null){
                                                                echo '未指定結束日期';
                                                            }else{
                                                ...
                                                <div class="row" style=" margin-top: 10px; margin-bottom: 10px; font-family:'Times New Roman','標楷體';">
                                                    ...
                                                    <div class="col" style="justify-content:center;">
                                                        <?php 
                                                            if($_SESSION['attendance_record_name'] == null){
                                                                echo '未指定病患姓名';
                                                            }else{
                                                ...
                                                <!-- TEST TEST TEST-->
                                                    <?php
                                                        if($_SESSION['attendance_record_name'] == null){
                                                            //echo '<script type="text/javascript">alert("請輸入查詢姓名")</script>';
                                                        }
                                                    ?>
                                                <!-- TEST TEST TEST-->
                                            </div>
                                        ...
                                        <!-- 原本按鈕觸發下載excel的程式
                                            document.getElementById('export_btn').addEventListener('click', function(){
                                                ...
                                                table2excel.export(document.querySelectorAll("#attendance_record_table"));
                                            })
                                        -->
                                        <div class="modal-footer">
                                            ...
                                            <script>
                                                ...
                                                {
                                                    var name2='<?php echo $_SESSION['attendance_record_name'];?>';
                                                    var start2='<?php echo $_SESSION['attendance_record_start_date'];?>';
                                                    var end2='<?php echo $_SESSION['attendance_record_end_date'];?>';
                                            ...
                                            <?php
                                                $_SESSION['attendance_record_name'] = '';
                                                $_SESSION['attendance_record_start_date'] = '';
                                                $_SESSION['attendance_record_end_date'] = '';
                                            ?>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="modal fade" id="staticBackdrop" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" style="font-family:'Times New Roman','標楷體';">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="./attendance_record_insert_patient.php" method="post">
                        <div class="container-fluid">                                            
                            <td>
                                病患姓名
